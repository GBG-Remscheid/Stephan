...
export abstract class Vertretung {
    ...
    async vertretung(
        ...
        let embed = new MessageEmbed()
            ...
            .setFooter({ text: `Requested by ${interaction.user.username}`, iconURL: interaction.user.avatarURL({ dynamic: true }) ?? interaction.user.defaultAvatarURL })
            .setColor("RANDOM")
            .setAuthor({ name: `Die Informationen von Untis können manchmal nicht stimmen. Frag am besten direkt bei deinem Fachlehrer nach, wenn du dir unsicher bist.` });

        untis.login().then(async () => {

            if (tag === Tag.heute) {
                timetable = await untis.getTimetableForToday(parseInt(klasse), 1);
            } else if (tag === Tag.morgen) {
                const date = new Date();
                const tommorow = date.setDate(date.getDate() + 1);
                const tommorowDate = new Date(tommorow);

                timetable = await untis.getTimetableFor(tommorowDate, parseInt(klasse), 1);
            }
            ...
            if (ausfall.map(lesson => lesson.substText).length > 0) {
                ...
                ausfall.map(lesson => {
                    embed.addField("Fach", `\`\`\`${formatLessonName(lesson.sg) ?? lesson.su.map(l => l.name) ?? "N/A"}\`\`\``, true)
                        .addField("Anmerkung", `\`\`\`${lesson.substText ?? "N/A"}\`\`\``, true)
                        .addField('\u200B', '\u200B', true)
